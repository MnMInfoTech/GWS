...
namespace MnM.GWS
{
    ...
    public
        ...
        sealed class PolyFill : _PolyFill, IPolyFill
        {
            ...
            public override void Fill(IWritable buffer, IReadable pen, LineCommand lineCommand = 0)
            {
                if (buffer == null)
                    return;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public override void FillLine(IWritable buffer, IReadable pen, ICollection<float> data, int axis, bool horizontal, float? alpha = null)
            {
                if (data == null || data.Count == 0)
                    return;
