...
namespace MnM.GWS
{
        ...
        partial class Brush : IBrush, ITextureBrush, IResizable
    {
        ...
        {
            int color = 0;
            ReadPixel2(x, y, session, ref color);
            return color;
        }
        unsafe partial void ReadPixel2(int x, int y, IReadSession session, ref int color);
        #endregion

        #region READ LINE
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void ReadLine(int Start, int End, int Axis, bool Horizontal, out int[] pixels, out int srcIndex, out int copyLength, IReadSession session)
        {
            pixels = null;
            srcIndex = 0;
            copyLength = 0;
            ReadLine2(Start, End, Axis, Horizontal, session, ref pixels, ref srcIndex, ref copyLength);
        }
        unsafe partial void ReadLine2(int Start, int End, int Axis, bool Horizontal, IReadSession session,
