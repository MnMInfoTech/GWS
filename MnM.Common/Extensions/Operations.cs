...
namespace MnM.GWS
{
    public static class Operations
    {
        ...
        {
            var t = typeof(T);
            keywords.Add(ExprType.Class, Enumerables.PrependItem(Enumerables.PrependItem(keys, t.Name), t.FullName));
        }
        ...
        {
            var t = typeof(T);
            keywords.Add(ExprType.Array, Enumerables.PrependItem(Enumerables.PrependItem(keys, t.Name), t.FullName));
        }
        ...
        public static IEnumerable<Genre> AddAssembley(this IKeywords keywords, Assembly assembly, Type whereBaseTypeIsThis = null)
        {
            ...
            var glist = types.Where(x => p(x)).Select(y => keywords.NewGenre(y));
        ...
        public static void AddAssembley(this IKeywords keywords, ref Genres genres, Assembly assembly, Type whereBaseTypeIsThis = null)
        {
            var list = keywords.AddAssembley(assembly, whereBaseTypeIsThis);
            genres.AddRange(list);
        }
        public static IEnumerable<Genre> AddAssembley(this IKeywords keywords, Type assemblyOfThis, Type whereBaseTypeIsThis = null) =>
            keywords.AddAssembley(Assembly.GetAssembly(assemblyOfThis), whereBaseTypeIsThis);
        public static IEnumerable<Genre> AddAssembley<AssemblyOfThis, IncludeOnlyDerivedTypeOfThis>(this IKeywords keywords) =>
           keywords.AddAssembley(Assembly.GetAssembly(typeof(AssemblyOfThis)), typeof(IncludeOnlyDerivedTypeOfThis));
        public static IEnumerable<Genre> AddAssembley<AssemblyOfThis>(this IKeywords keywords) =>
            keywords.AddAssembley(Assembly.GetAssembly(typeof(AssemblyOfThis)));

        public static IKeyword GetKeyword(this IKeywords keywords, Genres genres, string expression, string nameSpace = null)
        {
        ...
        public static object GetValue(this object source, string memberName, int? index = null)
        {
            ...
            if (index == null)
            {
                if (source is DataRow)
                    Dbs.ValueOf(source as DataRow, memberName, out value);
                else if (source is DataRowView)
                    Dbs.ValueOf((source as DataRowView), memberName, out value);
                else if (source is ICustomTypeDescriptor)
                {
                    var property = (source as ICustomTypeDescriptor).GetProperties().Find(memberName, true);
                    if (property != null) value = property.GetValue(source);
                }
            ...
            else
            {
                ...
                else if (source is IListSource)
                {
                    ...
                    if (_source.ContainsListCollection) return _source.GetList()[index.Value].GetValue(memberName);
                }
        ...
        public static object[] GetValues(this object source, string memberName, bool distinct = true)
        {
            ...
            {
                var _source = source as IListSource;
                if (_source.ContainsListCollection) items = _source.GetList();
            }
            else if (source is IEnumerable)
        ...
        public static void SetValue(this object source, string memberName, object value, int? index = null)
        {
            ...
            if (index == null)
            {
                ...
                {
                    var property = (source as ICustomTypeDescriptor).GetProperties().Find(memberName, true);
                    if (property != null) property.SetValue(source, value);
                }
                else if (source.IsPrimitive())
        ...
        public static T ConvertTo<T>(this object value)
        {
            ...
            T result;
            value.ConvertTo<T>(out result, null);
            return result;
        }
        ...
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public static bool ConvertTo<T>(this object value, out T result, Type intendedType, T defaultValue = default(T))
        {
            result = defaultValue;
            if (value == null)
                return false;
            ...
            else if (value is IEnumerable)
            {
                ...
                else if (value is byte[])
                {
                    if (intendedType == typeof(string))
                        value = (value as byte[]).ToBase64String();
                    else if (intendedType.ToTypeCode() == TypeCode.Object)
                        value = (value as byte[]).ToObject();
                ...
                {
                    if (value is Enum)
                        value = Enums.EnumName((Enum)value);
            ...
            else if (value is Type)
            {
                if (intendedType == typeof(Genre))
                    value = new Genre((Type)value, false, false);
                else if (intendedType == typeof(TypeCode))
                    value = (value as Type).ToTypeCode();
                else if (intendedType == typeof(string))
                    value = new Genre((Type)value, false, false).TypeName;
            }
            else if (value is Genre)
            {
                if (intendedType == typeof(Type))
                    value = (value as Genre).Value;
                else if (intendedType == typeof(TypeCode))
                    value = (value as Genre).Value.ToTypeCode();
                else if (intendedType == typeof(string))
                    value = (value as Genre).TypeName;
            }
            else if (value is TypeCode)
            {
                if (intendedType == typeof(Genre))
                    value = new Genre(((TypeCode)value).ToType(), false, false);
                else if (intendedType == typeof(Type))
                    value = ((TypeCode)value).ToType();
                else if (intendedType == typeof(string))
        ...
        /// <returns>System.Object.</returns>
        public static object MyInstance(this Type type, BindingFlagType flagtype,
           Type[] types, object[] args, Binder binder, ParameterModifier[] modifier)
        {
            try
            {
                ConstructorInfo ci = type.GetConstructor
                   (flagtype.Value(), binder, types, modifier);

                if (ci == null)
                {
                    ci = type.GetConstructor(flagtype.Value(),
                       binder, new Type[] { typeof(object[]) }, modifier);

                    if (ci != null) { args = new object[] { args }; }
                }
                if (ci != null)
                {
        ...
        /// <returns>System.Object.</returns>
        public static object MyInstance(this Type type, BindingFlagType flagtype,
                 object[] args, Binder binder, ParameterModifier[] modifier)
        {
            try
            {
                ConstructorInfo ci = type.GetConstructor
                   (flagtype.Value(), binder, args.GetTypes(), modifier);
                if (ci == null)
                {
                    ci = type.GetConstructor(flagtype.Value(),
                       binder, new Type[] { typeof(object[]) }, modifier);
        ...
        public static object MyInstance(this Type type, BindingFlagType flagtype, params object[] args)
        {
            ...
            {
                ConstructorInfo ci = type.GetConstructor
                   (flagtype.Value(), null, args.GetTypes(), null);
                if (ci == null)
                {
                    ci = type.GetConstructor(flagtype.Value(),
        ...
        public static object MyInstance(this Type type, BindingFlagType flagtype, Type[] types, object[] args)
        {
            ...
            {
                ConstructorInfo ci = type.GetConstructor
                   (flagtype.Value(), null, types, null);
        ...
        /// <returns>System.Object.</returns>
        public static object MyInstance(this Type myType, Instance mi, Type[] t, object[] arguments)
        {
            ConstructorInfo ci = null;
            try
            {
                ci = myType.GetConstructor(BindingFlagType.AllMember.Value(), null, t, null);
            }
            catch { }

            if (ci == null)
            {
                ci = myType.GetConstructor(BindingFlagType.AllMember.Value(), null, new Type[] { typeof(object[]) }, null);
        ...
        public static PropertyInfo MyProperty(this Type t, string name, bool ignoreCase, params object[] arguments)
        {
            ...
            if (!ignoreCase)
            {
                property = t.GetProperty(name, types);
            }
            else
            {
                BindingFlags flag = BindingFlagType.AllProperties.Value();
                var properties = t.GetProperties(flag);
        ...
        {
            if (t == null) return null;
            var properties = t.GetProperties(BindingFlagType.AllProperties.Value());
        ...
        {
            if (t == null) return null;
            return t.GetProperties(BindingFlagType.AllProperties.Value());
        }
        ...
        public static Tuple<string, object>[] PropertyValues(this object source)
        {
            ...
            var properties = type.GetProperties(findAllBinding);
        ...
        public static Type[] GetBaseTypes(this Type target)
        {
            ...
            Type t = target.BaseType;
            var l = new Collection<Type>();
        ...
        {
            var mytypes = new Collection<Type>(5);
            ...
            switch (option)
            {
                case ExtractInterfaces.TheseOnly:
                    ...
                    {
                        mytypes.Add(interfaces.FirstOrDefault(x =>
                            x.IsGenericType ? x.GetGenericTypeDefinition() == t : x == t));
                ...
                case ExtractInterfaces.ExcludeThese:
                    ...
                    {
                        if (!types.Any(x => x.IsGenericType ? x.GetGenericTypeDefinition() == t : x == t))
                            mytypes.Add(t);
                    }
                    break;
            ...
            if (arguments.Length > 0)
            {
                ...
                if (makeTheseByRef != null && makeTheseByRef.Length > 0)
                {
                    for (int i = 0; i < arguments.Length; i++)
                    {
                        ...
                        {
                            if (arguments[i] == null)
                                t[i] = Genre.Of(arguments[i], false, false).Value.MakeByRefType();
                            else
                                t[i] = arguments[i].GetType().MakeByRefType();
                        }
                        else if (arguments[i] == null)
                            t[i] = Genre.Of(arguments[i], false, false).Value;
                        else
        ...
        public static Collection<object> GetObjetArrayList(IEnumerable collection)
        {
            var array = new Collection<object>();
        ...
        internal static object InvokeMethod(this object o, string Name, Type[] types, params object[] arguments)
        {
            ...
            BindingFlags.NonPublic | BindingFlags.Public, null, types, null);
            if (m != null)
            {
                if (m.IsStatic)
                {
        ...
        /// <param name="b">The b.</param>
        /// <returns>System.Object.</returns>
        public static object ToObject(this byte[] b)
        {
        ...
        static bool ConvertFromString<T>(this string value, out T result)
        {
            ...
            {
                t = Genre.Of(t).GenricParams[0].Value;
