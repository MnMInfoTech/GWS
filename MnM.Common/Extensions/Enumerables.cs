...
namespace MnM.GWS
{
    public static partial class Enumerables
    {
        ...
        public static IReadOnlyList<T> Where<T>(this IEnumerable<T> source, IEnumerable<int> indices, CriteriaMode mode)
        {
            ...
            return new ReadOnlyList<T>(source.Where
                ((x, i) => (indexer[i] == !except)));
        }
        ...
        {
            range = new Span(range, source.Count());
            return new ReadOnlyList<T>(source.Where((x, i) => (range.Contains(i))));
        }
        ...
        public static IReadOnlyList<T> Where<T>(this IEnumerable<T> source, Criteria criteria, object value)
        {
            ...
            else { return new ReadOnlyList<T>(new T[0]); }
        }
        ...
        public static IReadOnlyList<T> Where<T>(this IEnumerable<T> source, MultCriteria criteria, object value1, object value2)
        {
            ...
            { return new ReadOnlyList<T>(new T[0]); }
        }
        ...
        /// <param name="index">The index.</param>
        /// <returns>System.Int32.</returns>
        public static int CopyTo<T>(this IEnumerable<T> collection, ref T[] target, ISpan range, int index)
        {
            if (target == null)
                target = new T[0];
            ...
            if (collection != null && index >= 0 && index <= target.Length)
            {
                int resize = 0;

                if (collection is T[] || collection is IArray<T>)
                {
                    int start = 0, count = 0;
                    ...
                    {
                        array = (collection as IArray<T>).Data;
                        count = (collection as IArray<T>).Count;
                    }
                    ...
                    {
                        resize = index + count - target.Length;
                        Array.Resize(ref target, target.Length + resize);
                    }
                    Array.Copy(array, start, target, index, count);
                }
                else if (collection is IList<T>)
                {
                    ...
                    if (range != null)
                    {
                        ...
                        if (index + r.Count > target.Length)
                        {
                            resize = index + r.Count - target.Length;
                            Array.Resize(ref target, target.Length + resize);
                        }
                        foreach (var i in r)
                        {
                            target[index] = list[i]; index++;
                        }
                    }
                    else
                    {
                        if (index + list.Count > target.Length)
                        {
                            resize = index + list.Count - target.Length;
                            Array.Resize(ref target, target.Length + resize);
                        }
                ...
                else
                {
                    ...
                    {
                        array = collection.Where(range).ToArray();
                    }
                    resize = index + array.Length - target.Length;
                    Array.Resize(ref target, target.Length + resize);
            ...
            ref T item, ISpan range, Criteria criteria, bool last, out bool isArray)
        {
            ...
            if ((collection is T[] || collection is IArray<T>) && criteria == Criteria.Equal)
            {
